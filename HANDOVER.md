...
⚠️ **迁移漏项补记（2026-09-23 实刮暴露）**：config.json 里的 `proxy` 原是 docker 容器名
`http://mdcx-clash:7890`，原生环境解析不了 → 所有走代理的刮削源报
`curl: (5) Could not resolve proxy`。已改为 `http://127.0.0.1:7890` 并重启
（备份 `config.json.bak-proxyfix`；**回退 docker 版时记得改回容器名**）。
实刮验证：NPJS-268 两个文件 2/2 成功，封面/NFO 落位 `整理完成/未知演员/`。
**教训：从容器迁原生，凡 config 里引用容器网络名的东西都要过一遍**（proxy、emby_url 等）。
